...
from numpy import array, ones, zeros, arange, uint8, argmax
...
def show_results(char,model):
    ...
    for i,ch in enumerate(char): #iterating over the characters
        ...
        # Use predict instead of deprecated predict_classes
        predictions = model.predict(img, verbose=0)
        y_ = argmax(predictions[0]) #predicting the class
        #print(y_)
        character = dic[y_] #
        if(character=="T"):
...
def draw_text_on_image(img,title,text,x=150,y=250,w=500,h=100):
    ...
    # Ensure x, y, w, h are valid integers
    x = int(x) if x is not None else 150
    y = int(y) if y is not None else 250
    w = int(w) if w is not None else 500
    h = int(h) if h is not None else 100
...
def LP_recognition(img,newImg,top):
	...
	if img is None or (hasattr(img, 'size') and img.size == 0):
		raise ValueError("Invalid license plate image provided for recognition")
	
	char=histogram_of_pixel_projection(img)
	
	# Check if any characters were segmented
	if len(char) == 0:
		print("Warning: No characters could be segmented from the license plate")
		...
		# Use default position if top is None or 0
		top_position = top if top is not None and top > 0 else newImg_height // 2
		final_img = draw_text_on_image(newImg, title, text, newImg_width//2-200, top_position//2-50, newImg_width//2+25, 100)
		return final_img
	
	#model_requirements()    #tf.keras.models.load_model
	model_path = os.path.join(project_root, "Licence_Plate_Recognition", "ocrmodel.h5")
	...
	# Handle None or invalid top value - use default position
